...
Status: consolidated design note as of 2026-04-23.

This repository prototypes a ScreenCast portal extension for OBS and other
multi-source capture clients. It adds two optional `SelectSources()` options:
...
Backends own presentation and backend-specific recovery. XDPH forwards
`source_label` to its picker. Cosmic goes further: when the token resolves to
restore data but the target window is not currently available, it can register
a rescue hook, wait for the window to reappear, and then send a late successful
`Start` response. This is backend value-add, not part of the generic portal
contract.
...
pop unexpected pickers after upgrade.
...
`source_label` sanitation is uneven by layer. The base portal validates length,
libportal truncates, Cosmic strips control characters, and XDPH forwards the
label to the picker environment. The spec should state which layer is
authoritative and which checks are mandatory versus display-only hygiene.
...
pending-rescue cap are pragmatic, not protocol-level guarantees.
...
mock crate README/tests. The older files are still useful as detailed history,
but `DESIGN.md` should be treated as the concise current design summary.
